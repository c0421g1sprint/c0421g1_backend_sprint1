...
public interface IMarkRepository extends JpaRepository<Mark, Integer> {
    ...
    //  MinhNN 27/10
    @Query(value = "SELECT mark.mark_id, mark.mark_point_number1, mark.mark_point_number2, mark.mark_point_number3, s.student_id, sb.subject_id, c.classroom_id\n" +
            "FROM mark\n" +
            ...
            "WHERE (?1 IS NULL OR s.student_name LIKE %?1%) AND  (?2 IS NULL OR sb.subject_id = ?2) AND (?3 IS NULL OR c.classroom_id = ?3)",
            countQuery = "SELECT COUNT(*) FROM mark " +
                    ...
                    "AND (?3 IS NULL OR c.classroom_id = ?3)"
            ,nativeQuery = true)
    Page<Mark> findAllStudentByName(Pageable pageable,String nameStudent, Integer id, Integer classID);
}