...
//  MinhNN 27/10
    @Query(value = "SELECT *\n" +
            ...
            "WHERE (?1 IS NULL OR s.student_name LIKE %?1%) AND  (?2 IS NULL OR sb.subject_name = ?2)",
            countQuery = "SELECT COUNT(*) FROM mark " +
                    "JOIN student s on mark.student_id = s.student_id\n" +
                    "JOIN subject sb on mark.subject_id = sb.subject_id\n" +
                    "WHERE (?1 IS NULL OR s.student_name LIKE %?1%)" +
                    "AND  (?2 IS NULL OR sb.subject_name = ?2)"

            ,nativeQuery = true)
    Page<Mark> findAllStudentByName(Pageable pageable,String nameStudent, String subject);
}
