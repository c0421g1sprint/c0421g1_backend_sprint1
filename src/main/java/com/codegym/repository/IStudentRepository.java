...
public interface IStudentRepository extends JpaRepository<Student, Integer> {
    ...
    @Modifying
    @Query(value = "update `student` set student_address = ?1, student_date_of_birth = ?2, student_ethnicity = ?3," +
    ...
    //Phuc xem thong tin hoc sinh cua giao vien chu nhiem chỉnh lại:
    @Query(value = " select st.student_id as studentId,st.student_name as studentName, st.delete_flag as studentFl, st.student_address as studentAddress, \n" +
            "            st.student_date_of_birth as studentDateOfBirth, cl.classroom_name as classroomName\n" +
            "             from student st join classroom cl on st.classroom_id = cl.classroom_id join teacher t on cl.teacher_id = t.teacher_id \n" +
            "            where t.teacher_id = :id ",
            countQuery = " select st.student_id as studentId, st.delete_flag as studentFl, st.student_address as studentAddress, \n" +
                    "            st.student_date_of_birth as studentDateOfBirth, cl.classroom_name as classroomName\n" +
                    "             from student st join classroom cl on st.classroom_id = cl.classroom_id join teacher t on cl.teacher_id = t.teacher_id \n" +
                    "            where t.teacher_id = :id  ", nativeQuery = true)
    Page<StudentListFromTeacher> getListStudent(Pageable pageable, @Param("id") Integer id);
